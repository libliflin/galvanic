    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 operates on 64-bit registers, so the primary instruction
                // cannot overflow at the 64-bit level. After the instruction, we
                // sign-extend the low 32 bits back to 64 bits (`sxtw x9, w{dst}`)
                // and compare with the full 64-bit result. If they differ, the
                // value exceeded the i32 range and we branch to `_galvanic_panic`.
                //
                // FLS §6.23 AMBIGUOUS: this guard fires for all 64-bit integer
                // computations, including i64 (false positive) and u32 wrap-around
                // (false negative, since u32 arithmetic should wrap, not panic).
                // Galvanic does not thread type information into IrBinOp; the guard
                // is applied uniformly. The same limitation exists for the MIN/-1
                // guard on sdiv/srem. Documented as a known limitation.
                //
                ...
                IrBinOp::Add => {
                    ...
                    writeln!(out, "    cmp     x{dst}, x9                      // FLS §6.23: overflow if 64-bit ≠ sext(32-bit)")?;
