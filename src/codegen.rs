    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 arithmetic uses 64-bit registers; i32 overflow produces a
                // value outside the i32 range. Guard: after the primary instruction,
                // sign-extend the low 32 bits (sxtw x9, w{dst}) and compare with
                // the full 64-bit result. If they differ, the result does not fit
                // in i32 → overflow → branch to _galvanic_panic.
                //
                // FLS §6.23 AMBIGUOUS: guard treats all arithmetic as i32 because
                // galvanic has no type system. False positives for i64 values outside
                // i32 range; false negatives for u32 wrap within [0, 2^32). At this
                // milestone, the test suite exercises i32; documented in
                // fls-ambiguities.md. Cache-line: 4 instructions (16 bytes) per op.
                IrBinOp::Add => {
