...
- `+`, `-`, `*` overflow: overflow guard added (Claim 4s). After each
  `add`/`sub`/`mul` x{dst}, a 3-instruction check emits `sxtw x9, w{dst}` /
  `cmp x{dst}, x9` / `b.ne _galvanic_panic`. Guard fires when the 64-bit result
  doesn't fit in i32, matching Rust debug-mode panic behaviour.
  FLS §6.23 AMBIGUOUS — spec requires debug panic / release wrap; galvanic
  always panics (no release-mode distinction). Also: for i64 operands this guard
  would produce false positives (valid i64 values outside i32 range); galvanic
  only applies it when the IR type is i32. For u32 operands the sxtw sign-
  extension is a semantic mismatch (u32 wrapping vs i32 panic); deferred.
