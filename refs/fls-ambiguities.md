...
- `cmp x{idx}, #len` (or `cmp x{idx}, x{len}` for runtime-length slices)
- `b.hs _galvanic_panic` — branches if index >= length (unsigned comparison, so
  negative indices also trigger the guard via wraparound).
