...
- `cmp x{idx}, #len` (or a runtime length load for slices)
- `b.hs _galvanic_panic` — branches on unsigned ≥, catching both too-large and
  negative indices (negative indices also trigger the guard via wraparound).
