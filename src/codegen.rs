    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // After each primary instruction, sign-extend the low 32 bits of the
                // result into a 64-bit scratch register and compare. If the 64-bit
                // result does not equal its own sign-extended 32-bit self, the result
                // does not fit in i32 → signed overflow → panic.
                //
                // FLS §6.23 AMBIGUOUS: galvanic has no type system, so the guard is
                // calibrated to i32. False positives for i64 values outside i32 range;
                // false negatives for u32 arithmetic that wraps within [0, 2^32).
                // At this milestone the test suite exercises i32; documented in
                // fls-ambiguities.md. Cache-line: four 4-byte instructions (16 bytes).
                IrBinOp::Add => {
                    writeln!(out, "    add     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: add")?;
                    // FLS §6.23: AMBIGUOUS — signed overflow: i32 overflow panics in debug mode.
                    // Guard fires for i64 values outside i32 range (false positive); does NOT
