...
- `cmp x{index}, #{len}` compares the index against the compile-time-known
  length (for arrays) or the runtime length slot (for slices).
- `b.hs _galvanic_panic` branches on unsigned ≥ (catches negative signed
  indices also, since they wrap to large unsigned values).
...
- `+`, `-`, `*` overflow (Claim 4s): guarded by `sxtw x9, w{dst}` +
  `cmp x{dst}, x9` + `b.ne _galvanic_panic` after every `add`, `sub`, and
  `mul` instruction. Fires when the 64-bit result does not equal its own
  sign-extended 32-bit self (i.e., the result does not fit in i32).
