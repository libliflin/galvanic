...
fn emit_instr(out: &mut String, instr: &Instr, frame_size: u32, saves_lr: bool, fn_name: &str, label_ctr: &mut usize) -> Result<(), CodegenError> {
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                // FLS §6.23 Claim 4s: Runtime overflow guard for i32 +, -, *.
                // ARM64 arithmetic uses 64-bit registers. After each primary instruction
                // we sign-extend the low 32 bits of the result (sxtw) and compare with
                // the full 64-bit result. If they differ, the value does not fit in i32
                // → overflow → panic. Guard sequence (4 instructions = 16 bytes):
                //   add/sub/mul x{dst}, x{lhs}, x{rhs}  — primary instruction
                //   sxtw x9, w{dst}                     — sign-extend low 32 bits
                //   cmp  x{dst}, x9                     — i32 overflow check
                //   b.ne _galvanic_panic                — overflow → panic
                // FLS §6.23 AMBIGUOUS: guard treats all arithmetic as i32 (no type
                // system). False positives for i64 values outside i32 range; false
                // negatives for u32 wrap within [0, 2^32). Acceptable at this milestone;
                // test suite exercises i32. Documented in fls-ambiguities.md.
                IrBinOp::Add => {
