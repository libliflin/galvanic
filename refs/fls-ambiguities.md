...
- `cmp x{idx}, x{len}` — compare index against length (unsigned)
- `b.hs _galvanic_panic` — out-of-bounds (index ≥ len) → panic

Negative indices (negative i64 values) cast to u64 become large unsigned
values that exceed the length, so the guard also catches them (negative
