...
fn emit_instr(out: &mut String, instr: &Instr, frame_size: u32, saves_lr: bool, fn_name: &str, label_ctr: &mut usize) -> Result<(), CodegenError> {
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                // FLS §6.23 Claim 4s: Arithmetic overflow guard for i32 +, -, *.
                //
                // The guard uses a post-instruction range check:
                //   1. Execute the operation into x{dst} (64-bit result).
                //   2. `sxtw x9, w{dst}` — sign-extend the low 32 bits to 64 bits.
                //   3. `cmp x{dst}, x9` — if equal, the result fits in i32; no overflow.
                //   4. `b.ne _galvanic_panic` — if they differ, the 64-bit result
                //      overflowed the i32 range → panic.
                //
                // FLS §6.23 AMBIGUOUS — i64/u32 false positives/negatives:
                // This guard is emitted for all uses of IrBinOp::Add/Sub/Mul,
                // including i64 and u32 operations. For i64, `sxtw` keeps only 32
                // bits, so valid i64 results outside i32 range will falsely panic
                // (false positive). For u32, values > i32::MAX will also falsely panic.
                // A type-threading pass would be required to suppress the guard for
                // non-i32 types — deferred to future work.
                //
                // Cache-line note: 4 instructions (16 bytes) per guarded arithmetic op.
                IrBinOp::Add => {
                    writeln!(out, "    add     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: add")?;
                    writeln!(out, "    sxtw    x9, w{dst}                       // FLS §6.23 Claim 4s: sign-extend lo32")?;
                    writeln!(out, "    cmp     x{dst}, x9                       // FLS §6.23 Claim 4s: overflow check")?;
                    writeln!(out, "    b.ne    _galvanic_panic                  // FLS §6.23 Claim 4s: panic on overflow")?;
                }
                IrBinOp::Sub => {
