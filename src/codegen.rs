    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 integer arithmetic uses 64-bit registers (xN), so overflow
                // does not wrap at 2^31 naturally. Guard: after the primary instruction,
                // sign-extend the low 32 bits of the result (sxtw x9, w{dst}) and
                // compare with the full 64-bit result. If they differ, the result does
                // not fit in i32 → overflow → branch to _galvanic_panic.
                //
                // FLS §6.23 AMBIGUOUS: guard treats all arithmetic as i32 semantics.
                // False positives for i64 values outside i32 range (galvanic has no
                // type system to distinguish). False negatives for u32 wrap within
                // [0, 2^32) but outside i32 range. At this milestone, the test suite
                // exercises i32; documented in refs/fls-ambiguities.md.
                IrBinOp::Add => {
