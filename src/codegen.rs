    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 integer arithmetic uses 64-bit registers (xN). After computing
                // the full 64-bit result, sign-extend the low 32 bits into x9. If the
                // result fits in i32, x{dst} and x9 are equal; if not, the 64-bit result
