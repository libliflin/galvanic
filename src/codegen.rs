...
fn emit_instr(out: &mut String, instr: &Instr, frame_size: u32, saves_lr: bool, fn_name: &str, label_ctr: &mut usize) -> Result<(), CodegenError> {
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                // FLS §6.23 Claim 4s: i32 arithmetic overflow guard.
                // ARM64 `add`/`sub`/`mul` operate on 64-bit registers, so they
                // never trap on i32 range overflow — the result is simply a
                // large 64-bit value. To detect i32 overflow in debug mode, we:
                //   1. Perform the 64-bit operation into x{dst}.
                //   2. Sign-extend the low 32 bits: `sxtw x9, w{dst}`.
                //   3. Compare the full 64-bit result to its sign-extended 32-bit
                //      projection: `cmp x{dst}, x9`.
                //   4. Branch to `_galvanic_panic` if they differ: `b.ne _galvanic_panic`.
                // If the result fits in i32, x{dst} and x9 are equal. If it does not
                // fit (overflow), they differ and the panic fires.
                //
                // FLS §6.23 AMBIGUOUS: the spec mandates debug-mode panic for all
                // signed integer types. Galvanic only guards i32 here; i64/u32
                // arithmetic does not receive an overflow guard at this milestone.
                // Cache-line note: 4 instructions × 4 bytes = 16 bytes per op.
                IrBinOp::Add => {
                    writeln!(out, "    add     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: add; §6.23 Claim 4s: overflow guard follows")?;
                    writeln!(out, "    sxtw    x9, w{dst}                       // FLS §6.23: sign-extend 32-bit result")?;
                    writeln!(out, "    cmp     x{dst}, x9                       // FLS §6.23: detect i32 overflow")?;
                    writeln!(out, "    b.ne    _galvanic_panic                  // FLS §6.23: overflow → panic")?;
                }
                IrBinOp::Sub => {
                    writeln!(out, "    sub     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: sub; §6.23 Claim 4s: overflow guard follows")?;
                    writeln!(out, "    sxtw    x9, w{dst}                       // FLS §6.23: sign-extend 32-bit result")?;
                    writeln!(out, "    cmp     x{dst}, x9                       // FLS §6.23: detect i32 overflow")?;
                    writeln!(out, "    b.ne    _galvanic_panic                  // FLS §6.23: overflow → panic")?;
                }
                IrBinOp::Mul => {
                    writeln!(out, "    mul     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: mul; §6.23 Claim 4s: overflow guard follows")?;
                    writeln!(out, "    sxtw    x9, w{dst}                       // FLS §6.23: sign-extend 32-bit result")?;
                    writeln!(out, "    cmp     x{dst}, x9                       // FLS §6.23: detect i32 overflow")?;
                    writeln!(out, "    b.ne    _galvanic_panic                  // FLS §6.23: overflow → panic")?;
                }
