    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 integer arithmetic uses 64-bit registers (xN). To detect
                // i32 overflow, galvanic uses the sxtw/cmp technique:
                ...
                //   4. b.ne _galvanic_panic — they differ → overflow → panic
                // FLS §6.23 AMBIGUOUS: spec requires debug-mode panic and
                // release-mode 32-bit wrap. Galvanic implements debug-mode panic
                // via sxtw/cmp. Release mode (no guard) is not yet implemented.
                // Cache-line note: 4 instructions = 16 bytes per arithmetic op.
                IrBinOp::Add => {
