    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // Claim 4s: overflow guard via post-instruction sign-extension check.
                // After each add/sub/mul, `sxtw x9, w{dst}` sign-extends the low 32
                // bits of the result; if the 64-bit result equals that sign-extended
                // value, it fits in i32 and no overflow occurred. Otherwise panic.
                //
                ...
                // i32 range; false negatives for u32 arithmetic that wraps within
                // [0, 2^32) but outside i32 range. Acceptable at this milestone —
