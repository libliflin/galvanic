    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 uses 64-bit registers; the result of `add`/`sub`/`mul` on
                // 64-bit operands does not wrap at i32 boundaries. We detect i32
                // overflow by sign-extending the lower 32 bits of the result back
                // to 64 bits (sxtw) and comparing with the full 64-bit result.
                // If they differ, the value did not fit in i32 → overflow → panic.
                //
                // Guard sequence (4 instructions = 16 bytes):
                //   add/sub/mul x{dst}, x{lhs}, x{rhs}  → 64-bit result
                //   sxtw x9, w{dst}                     → sign-extend lower 32 bits
                //   cmp  x{dst}, x9                     → overflow iff they differ
                //   b.ne _galvanic_panic                → panic on overflow
                //
                // FLS §6.23 AMBIGUOUS: the spec distinguishes debug-mode (panic) and
                // release-mode (two's complement 32-bit wrap); galvanic always panics.
                // FLS §6.23 AMBIGUOUS: the guard is only correct for i32 operands;
                // i64, u32, and other widths would require different sequences.
                // Cache-line: 4 instructions (16 bytes) per guarded op.
                IrBinOp::Add => {
                    writeln!(out, "    add     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: add; §6.23: Claim 4s overflow guard")?;
                    writeln!(out, "    sxtw    x9, w{dst}                      // §6.23: sign-extend lower 32 bits")?;
                    writeln!(out, "    cmp     x{dst}, x9                      // §6.23: overflow if 64-bit ≠ sext(32-bit)")?;
                    writeln!(out, "    b.ne    _galvanic_panic                 // §6.23: i32 overflow → panic")?;
                }
                IrBinOp::Sub => {
                    writeln!(out, "    sub     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: sub; §6.23: Claim 4s overflow guard")?;
                    writeln!(out, "    sxtw    x9, w{dst}                      // §6.23: sign-extend lower 32 bits")?;
                    writeln!(out, "    cmp     x{dst}, x9                      // §6.23: overflow if 64-bit ≠ sext(32-bit)")?;
                    writeln!(out, "    b.ne    _galvanic_panic                 // §6.23: i32 overflow → panic")?;
                }
                IrBinOp::Mul => {
                    writeln!(out, "    mul     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: mul; §6.23: Claim 4s overflow guard")?;
                    writeln!(out, "    sxtw    x9, w{dst}                      // §6.23: sign-extend lower 32 bits")?;
                    writeln!(out, "    cmp     x{dst}, x9                      // §6.23: overflow if 64-bit ≠ sext(32-bit)")?;
                    writeln!(out, "    b.ne    _galvanic_panic                 // §6.23: i32 overflow → panic")?;
                }
