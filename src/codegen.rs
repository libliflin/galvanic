    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                //
                // After the primary instruction, galvanic emits a 3-instruction guard:
                //   sxtw x9, w{dst}     → sign-extend low 32 bits of result into x9
                ...
                //
                // Rationale: i32::MAX + 1 produces 2147483648 (0x80000000) as a 64-bit
                // value, but sxtw(0x80000000) = -2147483648, so cmp fires → panic ✓.
                // Normal values (e.g. 100 + 200 = 300): sxtw(300) = 300 → no panic ✓.
                // Narrow types (u8, i8, u16, i16) stay within 32-bit range after masking;
                // sxtw matches the 64-bit value → no false positive ✓.
                //
                // FLS §6.23 AMBIGUOUS: guard treats all arithmetic as i32. False positives
                // for i64 values outside i32 range; false negatives for u32 wrap within
                // [0, 2^32) but outside i32 range. No type system to distinguish at this
                // milestone. Documented in refs/fls-ambiguities.md.
                //
                ...
                IrBinOp::Sub => {
                    ...
                    // FLS §6.23: AMBIGUOUS — same guard as Add above.
