...
- `cmp x{idx}, x{len}` compares the index against the array/slice length.
- `b.hs _galvanic_panic` branches to the panic trampoline if the index is
  out-of-bounds (unsigned ≥ comparison handles negative indices too — negative
...
- `+`, `-`, `*` overflow (Claim 4s): guarded by `sxtw x9, w{dst}` + `cmp x{dst}, x9`
  ...
  has no type system at codegen level. False positives for i64 values outside
  i32 range; false negatives for u32 arithmetic that wraps within [0, 2^32)
  but outside i32 range. At this milestone, the test suite exercises i32;
