...
- `cmp x{idx}, #len` (or `cmp x{idx}, x{len}` for slice) followed by `b.hs _galvanic_panic`.
- `b.hs` is unsigned ≥, so negative indices (reinterpreted as large unsigned values)
