    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 integer arithmetic uses 64-bit registers (xN), so these
                // instructions wrap at 2^64, not at i32::MAX (2^31-1). To detect
                // i32 overflow, after the 64-bit operation we sign-extend the low
                // 32 bits back to 64 bits and compare. If they differ, the result
                // overflowed i32 range — branch to _galvanic_panic.
                //
                ...
                // release-mode 32-bit wrap; galvanic always panics (debug-mode
                // behavior only). Cache-line: four 4-byte instructions (16 bytes) per op.
                IrBinOp::Add => {
                    writeln!(out, "    add     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: add; §6.23: 64-bit")?;
                ...
                IrBinOp::Sub => {
                    writeln!(out, "    sub     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: sub; §6.23: 64-bit")?;
                    writeln!(out, "    sxtw    x9, w{dst}                       // FLS §6.23: sign-extend low 32 bits")?;
                    writeln!(out, "    cmp     x{dst}, x9                       // FLS §6.23: i32 overflow check")?;
                    writeln!(out, "    b.ne    _galvanic_panic                  // FLS §6.23: overflow → panic")?;
                }
                IrBinOp::Mul => {
                    writeln!(out, "    mul     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: mul; §6.23: 64-bit")?;
                    writeln!(out, "    sxtw    x9, w{dst}                       // FLS §6.23: sign-extend low 32 bits")?;
                    writeln!(out, "    cmp     x{dst}, x9                       // FLS §6.23: i32 overflow check")?;
                    writeln!(out, "    b.ne    _galvanic_panic                  // FLS §6.23: overflow → panic")?;
                }
