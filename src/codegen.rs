    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 uses 64-bit registers, so add/sub/mul never trap.
                // Guard: after the instruction, sign-extend the low 32 bits and
                // compare with the full 64-bit result. If they differ, the i32
                // result doesn't fit and we panic per FLS §6.23 debug semantics.
                //
                // Sequence: <op> x{dst}; sxtw x9, w{dst}; cmp x{dst}, x9; b.ne _galvanic_panic
                //
                // FLS §6.23 AMBIGUOUS: guard treats all arithmetic as i32; will
                // false-positive for i64 values outside i32 range, and will
                // false-negative for u32 wrap within [i32::MAX+1, 2^32). At this
                // milestone galvanic has no type system; documented in fls-ambiguities.md.
                IrBinOp::Add => {
