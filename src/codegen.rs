    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 arithmetic uses 64-bit registers; i32 overflow does not trap.
                // Guard: after the primary instruction, sign-extend the low 32 bits
                // and compare with the 64-bit result. If they differ, the result does
                // not fit in i32 → panic.
                //
                ...
                // i32 range; false negatives for u32 values outside i32 range.
