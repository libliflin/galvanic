    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 arithmetic operates on 64-bit registers, so overflow is not
                // detected by hardware flags for 32-bit semantics. Galvanic inserts
                // an explicit check after each operation:
                //   1. Perform the 64-bit operation into x{dst}.
                //   2. Sign-extend the low 32 bits: sxtw x9, w{dst}
                //      (x9 = sign_extend(dst[31:0]) — what the result would be in i32)
                //   3. Compare: cmp x{dst}, x9
                //      If equal, the 64-bit result fits in i32 — no overflow.
                //      If not equal, the result overflowed i32 — panic.
                //   4. b.ne _galvanic_panic
                //
                // FLS §6.23 AMBIGUOUS: galvanic targets i32 semantics only. Operations
                // on i64 or u32 locals would incorrectly trigger this guard since those
                // types have different overflow ranges. Galvanic does not yet track value
                // types through IR — all arithmetic uses x-registers (64-bit). This guard
                // is correct for i32 programs but would be a false positive for i64.
                //
                // Cache-line note: 4 instructions (16 bytes) per guarded op.
                IrBinOp::Add => {
                    writeln!(out, "    add     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: add; §6.23 Claim 4s: i32 overflow guard")?;
                    writeln!(out, "    sxtw    x9, w{dst}                      // FLS §6.23: sign-extend 32-bit result")?;
                    writeln!(out, "    cmp     x{dst}, x9                      // FLS §6.23: 64-bit == sign_extend(32-bit)?")?;
                    writeln!(out, "    b.ne    _galvanic_panic                 // FLS §6.23: overflow → panic")?;
                }
                IrBinOp::Sub => {
                    writeln!(out, "    sub     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: sub; §6.23 Claim 4s: i32 overflow guard")?;
                    writeln!(out, "    sxtw    x9, w{dst}                      // FLS §6.23: sign-extend 32-bit result")?;
                    writeln!(out, "    cmp     x{dst}, x9                      // FLS §6.23: 64-bit == sign_extend(32-bit)?")?;
                    writeln!(out, "    b.ne    _galvanic_panic                 // FLS §6.23: overflow → panic")?;
                }
                IrBinOp::Mul => {
                    writeln!(out, "    mul     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: mul; §6.23 Claim 4s: i32 overflow guard")?;
                    writeln!(out, "    sxtw    x9, w{dst}                      // FLS §6.23: sign-extend 32-bit result")?;
                    writeln!(out, "    cmp     x{dst}, x9                      // FLS §6.23: 64-bit == sign_extend(32-bit)?")?;
                    writeln!(out, "    b.ne    _galvanic_panic                 // FLS §6.23: overflow → panic")?;
                }
