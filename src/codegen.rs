...
fn emit_instr(out: &mut String, instr: &Instr, frame_size: u32, saves_lr: bool, fn_name: &str, label_ctr: &mut usize) -> Result<(), CodegenError> {
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                // FLS §6.23: Claim 4s — signed integer overflow guard for +, -, *.
                //
                // ARM64 computes add/sub/mul in 64-bit registers (xN). A 64-bit
                // result that overflows i32 (> i32::MAX or < i32::MIN) will have
                // its low 32 bits differ from the sign-extended 32-bit value.
                //
                // Guard sequence (4 instructions = 16 bytes):
                //   add   x{dst}, x{lhs}, x{rhs}  → 64-bit result
                //   sxtw  x9, w{dst}               → sign-extend low 32 bits of dst
                //   cmp   x{dst}, x9               → compare full vs sign-extended
                //   b.ne  _galvanic_panic           → overflow if they differ
                //
                // This catches signed i32 overflow: if the result fits in i32, the
                // sxtw value equals the 64-bit value; if overflow, they differ.
                //
                // AMBIGUOUS: This guard also fires for i64/u32 computations that
                // happen to use the same `add`/`sub`/`mul` IR opcode, causing false
                // positives for those types. Galvanic's IR has no type annotations
                // on BinOp, so the guard is conservative. Documented as a known
                // deviation; a type-threaded IR would solve this.
                //
                ...
                IrBinOp::Add => {
                    ...
                    writeln!(out, "    cmp     x{dst}, x9                        // FLS §6.23: overflow if 64-bit ≠ sign-extended 32-bit")?;
                    writeln!(out, "    b.ne    _galvanic_panic                   // FLS §6.23: signed i32 overflow → panic")?;
                }
                IrBinOp::Sub => {
                    writeln!(
                        out,
