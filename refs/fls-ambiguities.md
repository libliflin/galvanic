...
check before the address computation: `cmp x{idx}, #len` / `b.hs _galvanic_panic`.
For slice indices with runtime length, the length is loaded from the fat-pointer
stack slot and used in the comparison. Negative indices (large unsigned values)
also trigger the guard via wraparound.
...
- `+`, `-`, `*` overflow (Claim 4s): guarded by `sxtw x9, w{dst}` + `cmp x{dst}, x9`
  ...
  False positives for i64 values outside i32 range; false negatives for u32
  arithmetic wrapping within [i32::MAX+1, 2^32). Test suite exercises i32;
