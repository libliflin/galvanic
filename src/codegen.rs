    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // result to 64 bits (sxtw). If the 64-bit result equals its own
                // sign-extended form, the value fits in i32 — no overflow. Otherwise
                // branch to _galvanic_panic.
                //
                // FLS §6.23 AMBIGUOUS: guard treats all arithmetic as i32.
                // False positives for i64 values outside i32 range; false negatives
                // for u32 wrap within [0, 2^32). Acceptable at this milestone —
