    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // Claim 4s: post-instruction i32 range check via sxtw.
                // `sxtw x9, w{dst}` sign-extends the low 32 bits of the result.
                // If the 64-bit result equals its own sign-extended 32-bit self,
                // the result fits in i32 — no overflow. Otherwise, panic.
                //
                // FLS §6.23 AMBIGUOUS: guard treats all arithmetic as i32 since
                // galvanic has no type system. False positives for i64 values
