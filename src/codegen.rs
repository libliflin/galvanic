    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // FLS §6.23 Claim 4s: signed arithmetic overflow guard.
                // After each add/sub/mul, emit a 3-instruction range check:
                //   sxtw x9, w{dst}   — sign-extend low 32 bits to 64 bits
                //   cmp  x{dst}, x9   — if 64-bit result ≠ sign-extended 32-bit, overflow
                //   b.ne _galvanic_panic  — i32 overflow → panic (exit 101)
                //
                // Why this works: if the true mathematical result fits in i32, then
                // w{dst} == the result (mod 2^32), and sxtw replicates the sign bit
                // to produce the same 64-bit value as x{dst}. If it overflowed, the
                // 64-bit result extends beyond ±2^31 and the two values differ.
                //
                // FLS §6.23 AMBIGUOUS: the guard treats all arithmetic as i32 because
                // galvanic has no type system at the codegen level. False positives for
                // i64 values outside i32 range; false negatives for u32 arithmetic that
                // wraps within [0, 2^32) but outside i32 range. At this milestone the
                // test suite exercises i32; documented in refs/fls-ambiguities.md.
                //
