...
fn emit_instr(out: &mut String, instr: &Instr, frame_size: u32, saves_lr: bool, fn_name: &str, label_ctr: &mut usize) -> Result<(), CodegenError> {
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                // FLS §6.23 Claim 4s: runtime i32 overflow guard for +, -, *.
                // ARM64 uses 64-bit registers (xN), so addition/subtraction/multiplication
                // wrap at 2^64, not at i32 boundaries. To detect i32 overflow we use a
                // post-instruction range check:
                ...
                // If the result fits in i32, sxtw(w{dst}) == x{dst} and no branch is taken.
                // FLS §6.23 AMBIGUOUS: guard calibrated to i32 semantics. False positives
                // for i64 values outside i32 range; false negatives for u32 wrap within
                // [i32::MAX+1, 2^32). Acceptable at this milestone — test suite exercises
                // i32; documented in fls-ambiguities.md. Cache-line: 4 instructions (16 bytes).
                IrBinOp::Add => {
