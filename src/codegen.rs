    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 arithmetic operates on 64-bit xN registers, so the raw
                // result may exceed the i32 range without wrapping. To detect
                // overflow, galvanic sign-extends the low 32 bits of the result
                // into a scratch register (x9) via `sxtw`, then compares the full
                // 64-bit result with the sign-extended 32-bit value. If they
                // differ, the result overflowed i32 and the program branches to
                // _galvanic_panic.
                //
                // FLS §6.23 AMBIGUOUS: the spec requires debug-mode panic for
                // integer overflow but does not specify the detection mechanism.
                // Galvanic uses sxtw/cmp as a sound implementation-defined choice.
                // Cache-line: 4 instructions × 4 bytes = 16 bytes per op.
                IrBinOp::Add => {
                    writeln!(
                        out,
                        "    add     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: add; §6.23: i32 overflow guard"
                    )?;
                    writeln!(out, "    sxtw    x9, w{dst}                        // §6.23: sign-extend 32-bit result to 64")?;
                    writeln!(out, "    cmp     x{dst}, x9                        // §6.23: overflow if 64-bit ≠ sign-ext-32")?;
                    writeln!(out, "    b.ne    _galvanic_panic                   // §6.23: overflow → panic")?;
                }
                IrBinOp::Sub => {
                    writeln!(
                        out,
                        "    sub     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: sub; §6.23: i32 overflow guard"
                    )?;
                    writeln!(out, "    sxtw    x9, w{dst}                        // §6.23: sign-extend 32-bit result to 64")?;
                    writeln!(out, "    cmp     x{dst}, x9                        // §6.23: overflow if 64-bit ≠ sign-ext-32")?;
                    writeln!(out, "    b.ne    _galvanic_panic                   // §6.23: overflow → panic")?;
                ...
                IrBinOp::Mul => {
                    writeln!(
                        out,
                        "    mul     x{dst}, x{lhs}, x{rhs}          // FLS §6.5.5: mul; §6.23: i32 overflow guard"
                    )?;
                    writeln!(out, "    sxtw    x9, w{dst}                        // §6.23: sign-extend 32-bit result to 64")?;
                    writeln!(out, "    cmp     x{dst}, x9                        // §6.23: overflow if 64-bit ≠ sign-ext-32")?;
                    writeln!(out, "    b.ne    _galvanic_panic                   // §6.23: overflow → panic")?;
