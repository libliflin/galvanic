    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 arithmetic uses 64-bit registers (xN). After each +/-/* we
                // sign-extend the low 32 bits and compare with the full 64-bit result.
                ...
                //   cmp  x{dst}, x9                         64-bit vs sign-extended
                //   b.ne _galvanic_panic                    overflow → panic
                //
                // FLS §6.23 AMBIGUOUS: guard treats all arithmetic as i32. False
                // positives for i64 values outside i32 range; false negatives for
                // u32 wrap within [0, 2^32). Galvanic has no type system to
                // distinguish. Documented in refs/fls-ambiguities.md.
                IrBinOp::Add => {
                ...
                IrBinOp::Sub => {
                    ...
                    writeln!(out, "    sxtw    x9, w{dst}                          // FLS §6.23: AMBIGUOUS — same as Add guard")?;
