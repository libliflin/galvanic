...
- `cmp x{idx}, x{len}` + `b.hs _galvanic_panic` before every load and store.
- Out-of-bounds branches to `_galvanic_panic` (negative indices also trigger
  the guard via wraparound).
