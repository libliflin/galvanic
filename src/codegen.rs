    ...
    match instr {
        ...
        Instr::BinOp { op, dst, lhs, rhs } => {
            match op {
                ...
                // ARM64 uses 64-bit registers; the primary op produces a 64-bit result.
                // Guard: sxtw sign-extends the low 32 bits to 64 bits. If the 64-bit
                // result equals its sign-extended 32-bit self, it fits in i32 (no
                // overflow). If they differ, the value overflowed i32 range → panic.
                //
                // FLS §6.23 AMBIGUOUS: the guard treats all arithmetic as i32 because
                // galvanic has no type system at codegen level. False positives for i64
                // values outside i32 range; false negatives for u32 arithmetic that wraps
                // within [0, 2^32) but outside i32 range. At this milestone, the test
                // suite exercises i32; documented in fls-ambiguities.md.
