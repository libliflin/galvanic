...
- `cmp x{index}, #N` (array length literal) or `ldr x9, [sp, #len_slot]` +
  `cmp x{index}, x9` (slice length from fat pointer)
- `b.hs _galvanic_panic` — branches if index ≥ length (unsigned compare, so
